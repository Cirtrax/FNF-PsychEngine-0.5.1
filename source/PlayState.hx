...
class PlayState extends MusicBeatState
{
	...
	//shit for da mod (most im tearing apart from bnb, so im crediting ash for this)
	...
	override public function create()
	{
				...
				case 'night':
					{
						...
						var bg1:FlxSprite = new FlxSprite(-970, -580).loadGraphic(Paths.image('split/BG1', 'shared'));
						...
						var bg2:FlxSprite = new FlxSprite(-1240, -650).loadGraphic(Paths.image('split/BG2', 'shared'));
						...
						var bg3:FlxSprite = new FlxSprite(-500, -330).loadGraphic(Paths.image('split/BG3', 'shared'));
						bg3.antialiasing = true;
						bg3.scale.set(1, 1);
						bg3.active = false;
						theEntireFuckingStage.add(bg3);
	
						var bg4:FlxSprite = new FlxSprite(-1390, -740).loadGraphic(Paths.image('split/BG4', 'shared'));
						...
						bg5.frames = Paths.getSparrowAtlas('split/pixelthing', 'shared');
		...
		areYouReady = new FlxTypedGroup<FlxSprite>();
			...
			for (i in 0...3) {
				...
				switch (i) {
					case 0:
						shit = new FlxSprite().loadGraphic(Paths.image('split/ARE', 'shared'));
					case 1:
						shit = new FlxSprite().loadGraphic(Paths.image('split/YOU', 'shared'));
					case 2:
						shit = new FlxSprite().loadGraphic(Paths.image('split/READY', 'shared'));
				}
				shit.cameras = [camHUD];
				shit.visible = false;
				areYouReady.add(shit);
			}
				//actually fucking insane for this shit but, whatever the typedgroup shit was being so fucking asinine to work with.

				light1 = new FlxSprite().loadGraphic(Paths.image('split/light0','shared'));
				light1.scrollFactor.set(0, 0);
				...
				light2 = new FlxSprite().loadGraphic(Paths.image('split/light1','shared'));
				...
				light3 = new FlxSprite().loadGraphic(Paths.image('split/light2','shared'));
				light3.scrollFactor.set(0, 0);
				light3.cameras = [camHUD];
				light3.alpha = 0;
				light3.updateHitbox();
				light3.antialiasing = true;
				add(light3);

				light4 = new FlxSprite().loadGraphic(Paths.image('split/light3','shared'));
				light4.scrollFactor.set(0, 0);
				light4.cameras = [camHUD];
				light4.alpha = 0;
				light4.updateHitbox();
				light4.antialiasing = true;
				add(light4);
				// glows
				glow = new FlxSprite().loadGraphic(Paths.image('split/Glow0','shared'));
				...
				glow1 = new FlxSprite().loadGraphic(Paths.image('split/Glow1','shared'));
				glow1.scrollFactor.set(0, 0);
				glow1.cameras = [camHUD];
				glow1.alpha = 0;
				glow1.updateHitbox();
				glow1.antialiasing = true;
				add(glow1);

				glow2 = new FlxSprite().loadGraphic(Paths.image('split/Glow2','shared'));
				...
				glow3 = new FlxSprite().loadGraphic(Paths.image('split/Glow3','shared'));
				glow3.scrollFactor.set(0, 0);
				glow3.cameras = [camHUD];
				glow3.alpha = 0;
				glow3.updateHitbox();
				glow3.antialiasing = true;
				add(glow3);
				//mirrored glows (actually i might redo this better yeah)
				/*var glowmirror:FlxSprite = new FlxSprite().loadGraphic(Paths.image('split/Glow0','shared'));
				glowmirror.scrollFactor.set(0, 0);
				glowmirror.cameras = [camHUD];
				glowmirror.alpha = 0;
				glowmirror.updateHitbox();
				glowmirror.antialiasing = true;
				add(glowmirror);

				var glowmirror1:FlxSprite = new FlxSprite().loadGraphic(Paths.image('split/Glow1','shared'));
				glowmirror1.scrollFactor.set(0, 0);
				glowmirror1.cameras = [camHUD];
				glowmirror1.alpha = 0;
				glowmirror1.flipX = true;
				glowmirror1.updateHitbox();
				glowmirror1.antialiasing = true;
				add(glowmirror1);

				var glowmirror2:FlxSprite = new FlxSprite().loadGraphic(Paths.image('split/Glow2','shared'));
				glowmirror2.scrollFactor.set(0, 0);
				glowmirror2.cameras = [camHUD];
				glowmirror2.alpha = 0;
				glowmirror2.flipX = true;
				glowmirror2.updateHitbox();
				glowmirror2.antialiasing = true;
				add(glowmirror2);

				var glowmirror3:FlxSprite = new FlxSprite().loadGraphic(Paths.image('split/Glow3','shared'));
				glowmirror3.scrollFactor.set(0, 0);
				glowmirror3.cameras = [camHUD];
				glowmirror3.alpha = 0;
				glowmirror3.updateHitbox();
				glowmirror3.antialiasing = true;
				add(glowmirror3);*/
		...
		else
		{
			...
			switch (curStage)
			{
				...
				case 'school' | 'schoolEvil':
					...
					camFollow.y = boyfriend.getMidpoint().y - 200;
				case 'night':
					camFollow.x = FlxMath.lerp(700, camFollow.x, 0.1);
					camFollow.y = FlxMath.lerp(475, camFollow.y, 0.1);
	...
	override function beatHit()
	{
		...
		if (curSong.toLowerCase() == 'split')
			switch (curBeat)
				{
					case 164, 168, 172, 176,
					 	195, 199, 203, 207, 211,
						228, 232, 236, 240, 244, 248, 252, 256:
						glow.flipX = false;				
						FlxTween.tween(glow, {alpha: 1}, 0.01, {ease: FlxEase.quadInOut});
						new FlxTimer().start(0.1, function(tmr:FlxTimer) {
							FlxTween.tween(glow, {alpha: 0}, 0.2, {ease: FlxEase.quadInOut});
						}); //this is a patchwork method to this shit since the code from bnb is ass to work with lmao
					case 165, 169, 173, 177, 
						196, 200, 204, 208,
						229, 233, 237, 241, 245, 249, 253, 257:	
						glow1.flipX = false;						
							FlxTween.tween(glow1, {alpha: 1}, 0.01, {ease: FlxEase.quadInOut});
							new FlxTimer().start(0.1, function(tmr:FlxTimer) {
								FlxTween.tween(glow1, {alpha: 0}, 0.2, {ease: FlxEase.quadInOut});
							}); //this is a patchwork method to this shit since the code from bnb is ass to work with lmao
					case 166, 170, 174, 178, 
						197, 201, 205, 209,
						230, 234, 238, 242, 246, 250, 254, 258:
						glow2.flipX = false;						
							FlxTween.tween(glow2, {alpha: 1}, 0.01, {ease: FlxEase.quadInOut});
							new FlxTimer().start(0.1, function(tmr:FlxTimer) {
								FlxTween.tween(glow2, {alpha: 0}, 0.2, {ease: FlxEase.quadInOut});
							}); //this is a patchwork method to this shit since the code from bnb is ass to work with lmao
					case 167, 171, 175, 179, 
						198, 202, 206, 210,
						231, 235, 239, 243, 247, 251, 255, 259:
						glow3.flipX = false;						
							FlxTween.tween(glow3, {alpha: 1}, 0.01, {ease: FlxEase.quadInOut});
							new FlxTimer().start(0.1, function(tmr:FlxTimer) {
								FlxTween.tween(glow3, {alpha: 0}, 0.2, {ease: FlxEase.quadInOut});
							}); //this is a patchwork method to this shit since the code from bnb is ass to work with lmao

					//bf shit		
					case 180, 184, 188, 192, 
						212, 216, 220, 224,
